...
all: build
	@echo "bloombox-pricelist is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-pricelist" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-pricelist"
endif
